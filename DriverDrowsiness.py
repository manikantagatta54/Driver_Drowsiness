...
def calculate_eye_aspect_ratio(eye_landmark, face_roi_landmark):
    ...
    center_top = calculate_midpoint(face_roi_landmark.part(eye_landmark[1]), face_roi_landmark.part(eye_landmark[2]))
    center_bottom = calculate_midpoint(face_roi_landmark.part(eye_landmark[5]), face_roi_landmark.part(eye_landmark[4]))
...
def calculate_mouth_aspect_ratio(lips_landmark, face_roi_landmark):
...
while True:
    ...
    for face_roi in faces:
        ...
        left_eye_ratio = calculate_eye_aspect_ratio([36, 37, 38, 39, 40, 41], landmark_list)
        right_eye_ratio = calculate_eye_aspect_ratio([42, 43, 44, 45, 46, 47], landmark_list)
        eye_open_ratio = (left_eye_ratio + right_eye_ratio) / 2
        cv2.putText(img, str(eye_open_ratio), (0, 13), font, 0.5, (100, 100, 100))

        inner_lip_ratio = calculate_mouth_aspect_ratio([60, 62, 64, 66], landmark_list)
        outer_lip_ratio = calculate_mouth_aspect_ratio([48, 51, 54, 57], landmark_list)
        ...
        x, y, x1, y1 = face_roi.left(), face_roi.top(), face_roi.right(), face_roi.bottom()
